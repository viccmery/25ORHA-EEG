function design = make_expdesign_eeg(subject_id,overwrite)
...
% experiment
design.n_runs                  = 8;
design.n_reps_per_run          = 2;
design.n_cat                   = 192; % number of categories in the stimulus set
design.n_trials_exp            = design.n_cat * design.n_reps_per_run; % number of trials for each run
design.n_trials_catch          = round((design.n_cat * design.n_reps_per_run)*0.20); % catch trials where response is required
...
max_position = design.n_trials_exp(i_run); % you don't want to insert beyond this
min_spacing = 4; % minimum number of exp trials between catch trials
max_spacing = 6; % maximum number of exp trials between catch trials

%generate catch trial positions for increased number of exp trials
while true
    catch_positions_absolute = [];
    current_pos = randi([min_spacing max_spacing]); % start somewhere after the first few trials

    while length(catch_positions_absolute) < design.n_trials_catch(i_run) && current_pos <= max_position
        catch_positions_absolute = [catch_positions_absolute current_pos];
        spacing = randi([min_spacing max_spacing]);
        current_pos = current_pos + spacing;
    end

    if length(catch_positions_absolute) >= design.n_trials_catch(i_run)
        % Trim if we somehow overshoot
        catch_positions_absolute = catch_positions_absolute(1:design.n_trials_catch(i_run));
        break;
    end
end

for i = length(catch_positions_absolute):-1:1
    design.trial_order = [design.trial_order(1:catch_positions_absolute(i)-1), ...
                          0, ...
                          design.trial_order(catch_positions_absolute(i):end)];
end
...
fn = mfilename;
fn = strrep(fn,'make_exp','');
fn = sprintf('%s_%s',fn,subject_id);
