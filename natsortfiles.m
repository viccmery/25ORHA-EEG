...
if nargin>1
	nsfChkRgx(rgx,chk)
	varargin = [{rgx},varargin(~(ivd|ive|ivp))];
end
%
%% Path and Extension %%
%
...
%
if any(ivd) % Remove "." and ".." folder names
...
%
if any(ive) % No file-extension
	fnm = nmx;
	fxt = [];
end
%
if any(ivp) % No file-path
	mat = reshape(fnm,1,[]);
else
