function ORHA_EEG_trig(subject_id, debug_mode)

%--------------------------------------------------------------------------
% EEG experiment for Object Representations in Healthy Aging (ORHA) project
%
% Original version: Johannes Singer
% 15.04.2024 : Marleen Haupt
% Current Version: Eddy Rhodes
%
% Last update: 21.06.2024
%
% Things to check before you use this script
% ER - I commented out the triggers and replaced them with an
% implementation that uses LabJack and can therefore be used from a laptop.
%
%       1. port
%       2. calculate image size in pixels
%       (depends on distance eye-monitor, image size and image pixels)
%
%--------------------------------------------------------------------------
%
% Input:
%       subject_id: subject identifier (string in '')
%       debug_mode: 1 = debugging mode, 0 = experiment mode (default: 0)
%
% Example call: ORHA_EEG('ORHA_999')
%
% Requirements before running this script:
%       design file for the same participant identifier in ./design/eeg/
%       (code: make_expdesign_eeg.m)
%       adjusted experimental settings in load_settings.m
%       visual angle calculation in visualanglecalculation.m then change
%       below (lines 78 etc.)
%
% Important variables:
%       exp: important setup variables
%       design: design and stimulus-relevant variables
%       results: results variables
%
% If you want to run this code on a computer that is not connected to an
% EEG system, please do not forget to comment out all %send_triggerIO64
% appearances to avoid error messages.
%--------------------------------------------------------------------------
...
exp.pix_per_deg = 75.78; %adapt this based on the visualanglecalculation script
exp.font_size = 36; %adapt this based on the visual needs of your participants
...
addpath(fullfile(exp.root_dir,'labjack'))

% Initiate LabJack
[params.ljasm, params.lhnd,params.lobj, params.ljerror] = LJinit();

% specify port address (see help document for more information)
% address=hex2dec('3FE0');

% set trigger delay time (if required)
exp.trigger_delay_time = 0.03;

% set initial condition number
% condition=99; % Not used so commented out
...
if exp.debug_mode == 0
    ...
    [exp.on_screen , exp.screen_rect] = Screen('OpenWindow', exp.screen_num);
...
for run_number = 1:design.n_runs
    ...
    LJtrigger_BP(params.lhnd,params.lobj,1,0.004,1);
    %-------------------------------------------------------------------------
    % start counting time
    %-------------------------------------------------------------------------
    ...
    for i_trial = 1:design.n_trials_total(run_number) % go through all trials
     ...
     if  trials(i_trial).image_nr< 193
         LJtrigger_BP(params.lhnd,params.lobj,4,0.004,1); % Trigger 4 for
         % normal trial
     else
         LJtrigger_BP(params.lhnd,params.lobj,8,0.004,1); % Trigger 8 for
         % catch trial
     end
     ...
     if ismember(exp.response_keys, key_id)
         ...
         LJtrigger_BP(params.lhnd,params.lobj,16,0.004,1);
     end
    ...
    LJtrigger_BP(params.lhnd,params.lobj,23,0.004,1);
